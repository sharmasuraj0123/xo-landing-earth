        ...
        <div className="marquee flex items-center gap-20 lg:gap-28 whitespace-nowrap w-max">
          {items.map((partner, i) =>
            partner.src ? (
              ...
              <span
                key={`${partner.name}-${i}`}
                className="text-xl lg:text-3xl font-display font-semibold tracking-tight text-foreground/50 hover:text-foreground/90 transition-colors duration-300 select-none"
              >
                {partner.name}
              </span>
            )
